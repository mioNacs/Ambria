...
export const TextEditor = React.forwardRef<TamboEditor, TextEditorProps>(
  (
    ...
    const stableSearchResources = React.useCallback(
      (query: string) => onSearchResources(query),
      [onSearchResources],
    );

    const stableSearchPrompts = React.useCallback(
      (query: string) => onSearchPrompts(query),
      [onSearchPrompts],
    );

    const handleResourceSelect = React.useCallback(
      (item: ResourceItem) => onResourceSelect(item),
      [onResourceSelect],
    );

    const handlePromptSelect = React.useCallback(
      (item: PromptItem) => onPromptSelect(item),
      [onPromptSelect],
    );
    ...
    const resourceSuggestion = React.useMemo<Omit<SuggestionOptions, "editor">>(
      () => ({
        char: "@",
        items: ({ query }) => {
          stableSearchResources(query);
          return [];
        },
        render: () => {
          ...
          return {
            ...
            onKeyDown: ({ event }) => {
              const { state, setState } = resourceRef.current;
      ...
      [stableSearchResources, handleResourceSelect, resourceRef],
    );

    const promptCommandExtension = React.useMemo(
      () =>
        Extension.create({
          name: "promptCommand",
              ...
              Suggestion({
                ...
                render: () => {
                  ...
                  return {
                    ...
                    onKeyDown: ({ event }) => {
                      const { state, setState } = promptRef.current;
                      if (!state.isOpen) return false;
      ...
      [stableSearchPrompts, handlePromptSelect, promptRef],
    );
